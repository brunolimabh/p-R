...
crop.data <- read.csv("C:/Users/bruno/Documents/projetosR/datasets/crop.data.csv", 
                      header = TRUE, 
...
two.way.plot <- two.way.plot +
                stat_summary(fun.data = 'mean_se', 
                             ...
                             stat_summary(fun.data = 'mean_se', 
                                          geom = 'pointrange') +
                                          geom_point(data=mean.yield.data, 
                                                     aes(x=density, 
                                                         y=yield))
two.way.plot

two.way.plot <- two.way.plot +
                geom_text(data=mean.yield.data, 
                          label=mean.yield.data$group, 
                          vjust = -8, 
                          size = 5) + 
